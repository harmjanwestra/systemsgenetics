...
//        Graphics2D g2d = bi.createGraphics();
        ...
        for(int i=0;i<header.length;i++){
            switch (header[i].toLowerCase()) {
                case "metap" -> pvalcol = i;
                case "snpchr" -> snpchrcol = i;
                case "snppos" -> snpposcol = i;
                case "genechr" -> genechrcol = i;
                case "genepos" -> geneposcol = i;
            }
        }

        if(pvalcol == -1 || snpchrcol == -1 || snpposcol == -1 || genechrcol == -1 || geneposcol == -1){
            if(pvalcol == -1){
                System.out.println("Error: missing column named MetaP");
            }
            if(snpchrcol == -1){
                System.out.println("Error: missing column named SNPChr");
            }
            if(snpposcol == -1){
                System.out.println("Error: missing column named SNPPos");
            }
            if(genechrcol == -1){
                System.out.println("Error: missing column named GeneChr");
            }
            if(geneposcol == -1){
                System.out.println("Error: missing column named GenePos");
            }
            System.exit(-1);
        }
        ...
        while ((str = in.readLine()) != null) {
            ...
            double pValue = Double.parseDouble(data[pvalcol]);
            if (pValue < pValueMin) {
                pValueMin = pValue;
            }
            if (pValue > pValueMax) {
        ...
        System.out.println(log10Min+"\t"+log10Max);
        if (log10Max > 65.90) {
            log10Max = 65.90;
        }
        ...
        while ((str = in.readLine()) != null) {
            ...
            int chrSNP = Integer.parseInt(data[snpchrcol]);
            int chrPosSNP = Integer.parseInt(data[snpposcol]);
            int chrProbe = Integer.parseInt(data[genechrcol]);
            int chrPosProbe = Integer.parseInt(data[geneposcol]);

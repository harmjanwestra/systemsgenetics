...
public class DeconvolutionTest {
    ...
    public void writeDeconvolutionResultWithSpearmanCorrelationTest() throws Exception {
        ...
        while (deconResults.hasNext() && deconExpected.hasNext()) {
            ...
            for (int q = 1; q < deconResultsStringVector.size(); q++) {
                double d = Double.parseDouble(deconResultsStringVector.get(q));
                deconResultsStringVector.set(q, df.format(d));
                d = Double.parseDouble(deconExpectedStringVector.get(q));
                deconExpectedStringVector.set(q, df.format(d));
            }
